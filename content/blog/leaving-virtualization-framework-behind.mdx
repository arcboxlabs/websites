...
## The platform doesn't always help

Even without the framework, the platform can fight you. Our development machines are M5 Max on macOS 26, released a couple of months ago, and on that combination the Hypervisor.framework guest is advertised as having **phantom SVE2.1**: the CPU feature bits show it, but the very first SVE instruction the guest tries to execute traps with SIGILL. This is a macOS bug in the virtualization layer that only surfaces on hardware new enough that Apple's own layers haven't fully caught up. The workaround is `arm64.nosve` on the kernel command line, unconditionally on M5. Building on the newest Apple Silicon means occasionally patching around what Apple gets wrong on top of what Apple doesn't do for you.
...
**Virtualization.framework is still the default.** Users pick between backends with `ARCBOX_VM_BACKEND`, and VZ remains the default while we close the last correctness gaps. The rest of the work is tracked in the [HV Backend project on Linear](https://linear.app/arcbox/project/hv-backend-35014c93f85e).
...
```bash
ARCBOX_VM_BACKEND=hv arcbox start
